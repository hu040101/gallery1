...
function App() {
  ...
  const handleUploadComplete = async (targetCountryId) => {
    await fetchCountries(targetCountryId);
  ...
  return (
    <>
      ...
      <header className="header" style={{ position: 'relative' }}>
        <h1>World Gallery</h1>
        <p>{IS_VIEWER ? 'A beautifully curated collection of places' : 'Your beautifully curated collection of places'}</p>
      </header>
        ...
        <main className="main-content glass-panel" style={{ padding: '2rem' }}>
          {activeCountry ? (
            ...
            <div style={{ textAlign: 'center', padding: '4rem 0', color: '#888' }}>
              {loading ? <div className="loading-spinner"></div> : (IS_VIEWER ? "No regions found." : "请在左侧选择或添加一个地区")}
            </div>
          )}
        </main>
      </div>
